    ...
    class CoverStyle
    {
        ...
        public void detectCoverStyle(List<Paragraph> list, WordprocessingDocument doc)
        {
            ...
            else
            {
                ...
                if (!(Util.getFullText(list[i]).Trim().Length - 16 == int.Parse(coverstyleHeadline[1]) || Util.getFullText(list[i]).Trim().Length - 10 == int.Parse(coverstyleHeadline[1]) + 4)) Util.printError("封面大标题中间不应多余包含空格且不应该缺少字符");
